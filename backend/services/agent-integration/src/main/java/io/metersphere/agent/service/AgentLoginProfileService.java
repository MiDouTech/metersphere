...
public class AgentLoginProfileService {
    ...
    private void validate(AgentLoginProfileRequest r,String projectId){AgentEnvironmentProfileDTO env=environments.get(projectId,r.getEnvironmentProfileId());environments.assertTargetAllowed(env,r.getLoginUrl());if(!LOGIN_TYPES.contains(r.getLoginType().toUpperCase()))throw new MSException("LOGIN_TYPE_INVALID");if(!MFA_POLICIES.contains(r.getMfaPolicy().toUpperCase()))throw new MSException("MFA_POLICY_INVALID");validateLocator(r.getUsernameLocator());validateLocator(r.getPasswordLocator());validateLocator(r.getSubmitLocator());validateAssertion(r.getSuccessAssertion());if(StringUtils.isNotBlank(r.getSessionValidation()))validateAssertion(r.getSessionValidation());}
