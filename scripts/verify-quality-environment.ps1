...
$sql = "SELECT COUNT(*) FROM metersphere_version WHERE success=0; SELECT COUNT(*) FROM metersphere_version WHERE version IN ('3.7.2.91','3.7.2.92','3.7.2.93','3.7.2.95','3.7.2.96','3.7.2.97') AND success=1 AND checksum IS NOT NULL; START TRANSACTION; INSERT INTO execution_quality_policy_project(project_id) VALUES ('quality-environment-smoke'); SELECT COUNT(*) FROM execution_quality_policy_project WHERE project_id='quality-environment-smoke'; ROLLBACK;"
...
if ($lines.Count -ne 3 -or $lines[0] -ne '0' -or $lines[1] -ne '6' -or $lines[2] -ne '1') {
