<template>
  <TestAssetPage>
    <MsCard simple>
      <div class="mb-4"
        ><div class="text-base font-medium">关联追溯</div
        ><div class="mt-1 text-sm text-[var(--color-text-3)]"
          >从业务文档、用例或任务任一端检索，查看来源与执行关系。</div
        ></div
      >
      <div class="mb-4 flex flex-wrap items-center gap-3">
        <a-input-search
          ...
          class="w-[240px]"
          allow-clear
          placeholder="搜索任一端名称或 ID"
          @search="search"
        />
        <a-select v-model="query.relationType" class="w-[170px]" allow-clear placeholder="关系类型" @change="search">
          <a-option value="DERIVED_FROM">来源于</a-option><a-option value="USES">使用</a-option
          ><a-option value="EXECUTES">执行</a-option><a-option value="COVERS">覆盖</a-option
          ><a-option value="PRODUCES">产出</a-option><a-option value="REPORTS">报告缺陷</a-option
          ><a-option value="REPLACES">替代</a-option>
        </a-select>
        <a-select v-model="query.assetType" class="w-[150px]" allow-clear placeholder="端点类型" @change="search"
          ><a-option value="DOCUMENT">文档</a-option><a-option value="CASE">用例</a-option
          ><a-option value="TASK">任务</a-option><a-option value="STEP">执行步骤</a-option
          ><a-option value="DATASET">测试数据</a-option><a-option value="ENVIRONMENT">环境</a-option
          ><a-option value="COMMON_STEP">公共步骤</a-option><a-option value="API_DEFINITION">接口</a-option
          ><a-option value="EVIDENCE">证据</a-option><a-option value="BUG">缺陷</a-option></a-select
        >
        <a-input
      ...
      >
        ...
        <template #columns>
          <a-table-column title="来源资产" :width="310"
          ...
          <a-table-column title="关系" :width="150"
            ><template #cell="{ record }"
              ><a-tag color="arcoblue">{{ record.relationType }}</a-tag></template
            ></a-table-column
          >
          <a-table-column title="目标资产" :width="310"
            ><template #cell="{ record }"
              ><AssetEndpoint
...
<script setup lang="ts">
  ...
  async function load() {
    ...
    loading.value = true;
    try {
      const result = await pageTestAssetRelations({
