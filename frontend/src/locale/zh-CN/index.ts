import common from './common';
import localeSettings from './settings';
import sys from './sys';
import dayjsLocale from 'dayjs/locale/zh-cn';

const _Cmodules: any = import.meta.glob('../../components/**/locale/zh-CN.ts', { eager: true });
const _Vmodules: any = import.meta.glob('../../views/**/locale/zh-CN.ts', { eager: true });
let result = {};
Object.keys(_Cmodules).forEach((key) => {
  const defaultModule = _Cmodules[key as any].default;
  if (!defaultModule) return;
  result = { ...result, ...defaultModule };
});
Object.keys(_Vmodules).forEach((key) => {
  const defaultModule = _Vmodules[key as any].default;
  if (!defaultModule) return;
  result = { ...result, ...defaultModule };
});
export default {
  message: {
    'menu.workbench': '工作台',
    'menu.workbenchHomeSort': '首页',
    'menu.workbenchWaitSort': '我的待办',
    'menu.workbenchFollowSort': '我关注的',
    'menu.workbenchCreatedSort': '我创建的',
    'menu.testPlan': '测试计划',
    'menu.testAsset': '测试资产',
    'menu.testAssetShort': '资产',
    'menu.testAsset.documents': '业务文档',
    'menu.testAsset.versions': '资产版本',
    'menu.testAsset.relations': '关联追溯',
    'menu.testAsset.cases': '用例资产',
    'menu.testAsset.datasets': '测试数据',
    'menu.testAsset.environments': '测试环境',
    'menu.testAsset.commonSteps': '公共步骤',
    'menu.testAsset.apis': '接口资产',
    'menu.testAsset.evidence': '执行证据',
    'menu.testAsset.bugs': '缺陷资产',
    'menu.testPlanGroup': '计划组',
    'menu.testPlanShort': '计划',
    'menu.testPlan.testPlanDetail': '测试计划详情',
    'menu.bugManagement': '缺陷管理',
    'menu.bugManagementShort': '缺陷',
    'menu.bugManagement.bugDetail': '缺陷',
    'menu.bugManagement.bugRecycle': '回收站',
    'menu.bugManagement.automationExecution': '自动化执行',
    'menu.caseManagement': '测试用例',
    'menu.caseManagementShort': '用例',
    'menu.apiTest': '接口测试',
    'menu.apiTestShort': '接口',
    'menu.apiTest.debug': '调试',
    'menu.apiTest.debug.debug': '调试',
    'menu.apiTest.management': '定义',
    'menu.apiTest.management.definition': '定义',
    'menu.apiTest.api': 'API列表',
    'menu.apiTest.apiScenario': '场景',
    'menu.apiTest.scenario': '场景',
    'menu.apiTest.scenario.recycle': '回收站',
    'menu.apiTest.report': '报告',
    'menu.apiTest.reportTestPlan': '计划报告',
    'menu.apiTest.reportTestGroupPlan': '计划组报告',
    'menu.apiTest.reportDetail': '报告详情',
    'menu.uiTest': 'UI测试',
    'menu.taskCenter': '任务中心',
    'menu.loadTest': '性能测试',
    'menu.performanceTest': '性能测试',
    'menu.projectManagement': '项目管理',
    'menu.projectManagementShort': '项目',
    'menu.projectManagement.projectList': '项目列表',
    'menu.projectManagement.templateManager': '模板管理',
    'menu.projectManagement.log': '日志',
    'menu.projectManagement.taskCenter': '任务中心',
    'menu.projectManagement.fileManagement': '文件管理',
    'menu.projectManagement.messageManagement': '消息管理',
    'menu.projectManagement.commonScript': '公共脚本',
    'menu.projectManagement.fakeError': '误报库',
    'menu.projectManagement.messageManagementEdit': '更新模板',
    'menu.projectManagement.environmentManagement': '环境管理',
    'menu.caseManagement.featureCase': '功能用例',
    'menu.caseManagement.executeCase': '执行用例',
    'menu.caseManagement.xmindCase': 'Xmind用例',
    'menu.caseManagement.caseGenerate': '生成用例',
    'menu.caseManagement.featureCaseRecycle': '回收站',
    'menu.caseManagement.featureCaseList': '用例列表',
    'menu.caseManagement.featureCaseDetail': '创建用例',
    'menu.caseManagement.featureCaseEdit': '更新用例',
    'menu.caseManagement.featureCaseCreateSuccess': '创建用例成功',
    'menu.caseManagement.caseManagementReview': '用例评审',
    'menu.caseManagement.caseManagementReviewShort': '评审',
    'menu.caseManagement.caseManagementReviewCreate': '创建评审',
    'menu.caseManagement.caseManagementReviewDetail': '评审详情',
    'menu.caseManagement.caseManagementReviewDetailCaseDetail': '评审用例详情',
    'menu.caseManagement.caseManagementCaseReviewEdit': '更新评审',
    'menu.caseManagement.caseManagementCaseDetail': '用例详情',
    'menu.caseManagement.testReport': '测试报告',
    'menu.caseManagement.testReportDetail': '报告详情',
    'menu.caseManagement.testReportEdit': '编辑报告',
    'menu.projectManagement.projectPermission': '项目与权限',
    'menu.settings': '系统设置',
    'menu.settingsShort': '系统',
    'menu.settings.system': '系统',
    'menu.settings.system.user': '用户',
    'menu.settings.system.usergroup': '用户组',
    'menu.settings.system.permissionControl': '权限控制',
    'menu.settings.system.authorizedManagement': '授权',
    'menu.settings.system.pluginManager': '插件',
    'menu.settings.system.organizationAndProject': '组织与项目',
    'menu.settings.system.orgStructure': '组织架构',
    'menu.settings.system.resourcePool': '资源池',
    'menu.settings.system.resourcePoolDetail': '添加资源池',
    'menu.settings.system.resourcePoolEdit': '编辑资源池',
    'menu.settings.system.parameter': '系统参数',
    'menu.settings.system.log': '日志',
    'menu.settings.system.agentIntegration': 'Agent 集成',
    'system.agentIntegration.createToken': '创建 Token',
    'system.agentIntegration.searchToken': '搜索 Token 名称',
    'system.agentIntegration.tokenName': 'Token 名称',
    'system.agentIntegration.tokenNameRequired': '请输入 Token 名称',
    'system.agentIntegration.userId': '关联用户（登录身份）',
    'system.agentIntegration.userIdRequired': '请选择关联用户',
    'system.agentIntegration.userIdPlaceholder': '默认当前用户；可搜索选择，或直接输入平台用户ID/企微UserID',
    'system.agentIntegration.userIdHelp':
      '用作 Agent API 的登录/执行身份凭证（不是管理员密码）。默认关联当前登录用户，可手动更改。',
    'system.agentIntegration.projectIds': '可访问项目',
    'system.agentIntegration.projectIdsHelp': '可多选；不选表示当前用户本人可访问的全部项目。',
    'system.agentIntegration.projectIdsPlaceholder': '不选 = 本人全部可访问项目',
    'system.agentIntegration.scopes': '权限范围',
    'system.agentIntegration.scopesRequired': '请选择权限范围',
    'system.agentIntegration.enable': '启用',
    'system.agentIntegration.tokenCreated': 'Token 创建成功',
    'system.agentIntegration.deleteConfirm': '确认删除 Token {name}？',
    'system.agentIntegration.mcpTitle': 'MCP 技能包',
    'system.agentIntegration.mcpDesc':
      '下载技能包交给 AI（如 Cursor），使其学会调用本平台 Agent/MCP 接口。平台侧只需：下载技能包 + 创建 Token。',
    'system.agentIntegration.mcpVersion': '当前包版本 {version}',
    'system.agentIntegration.mcpUnavailable': '技能包暂不可用，请联系管理员重新打包部署',
    'system.agentIntegration.mcpDownload': '下载 AI 技能包',
    'system.agentIntegration.mcpDownloadSuccess': '开始下载 AI 技能包',
    'system.agentIntegration.mcpHint':
      'AI 技能包用于指导 AI 连接远程 MCP，不包含 Token。创建 Token 后复制 MCP 配置到对应客户端。',
    'system.agentIntegration.platformAddress': '内置平台地址',
    'system.agentIntegration.platformTest': '测试环境',
    'system.agentIntegration.platformProd': '正式环境',
    'system.agentIntegration.mcpScopeHelp': '常用 Scope（创建 Token 时选择）',
    'system.agentIntegration.showConnectionDetails': '查看详细接入说明',
    'system.agentIntegration.hideConnectionDetails': '收起详细接入说明',
    'system.agentIntegration.scopeAgentAll': '全部 Agent 权限',
    'system.agentIntegration.scopeAgentAllDesc':
      '包含查询、执行、用例、计划、评审、缺陷和项目相关能力，仍受本人 RBAC 限制。',
    'system.agentIntegration.scopeBug': '缺陷管理',
    'system.agentIntegration.scopeBugDesc': '缺陷检索、详情查看、创建和更新。',
    'system.agentIntegration.scopeBugRead': '缺陷只读',
    'system.agentIntegration.scopeBugReadDesc': '仅允许检索和查看缺陷。',
    'system.agentIntegration.scopeBugExtend': '缺陷评论/附件/关联',
    'system.agentIntegration.scopeBugExtendDesc': '允许管理缺陷评论、缺陷详情附件和缺陷关联用例。',
    'system.agentIntegration.scopeBugDelete': '缺陷删除/恢复',
    'system.agentIntegration.scopeBugDeleteDesc': '允许删除至回收站和恢复缺陷；不包含永久删除。',
    'system.agentIntegration.scopeCase': '测试用例管理',
    'system.agentIntegration.scopeCaseDesc': '用例读取、结果提交、测试计划和评审相关操作。',
    'system.agentIntegration.scopeCaseMaintain': '测试用例维护',
    'system.agentIntegration.scopeCaseMaintainDesc': '允许更新用例、管理用例评论和用例详情附件，不包含删除。',
    'system.agentIntegration.scopeCaseDelete': '测试用例删除/恢复',
    'system.agentIntegration.scopeCaseDeleteDesc': '允许查询回收站、软删除和恢复用例；不包含永久删除。',
    'system.agentIntegration.scopeFunctionalRead': '功能测试只读',
    'system.agentIntegration.scopeFunctionalReadDesc': '仅允许读取功能用例、模块、计划和评审信息。',
    'system.agentIntegration.scopeProjectRead': '项目查看',
    'system.agentIntegration.scopeProjectReadDesc':
      '允许检索和查看可访问项目；迁移期 FUNCTIONAL_READ 仍临时覆盖本权限。',
    'system.agentIntegration.scopeFunctionalSubmit': '测试结果提交',
    'system.agentIntegration.scopeFunctionalSubmitDesc': '仅允许提交用例执行结果和执行附件。',
    'system.agentIntegration.myTokens': '我的 Agent Token',
    'system.agentIntegration.myTokensDesc': '用户可自行管理自己创建的 Token。',
    'system.agentIntegration.myTokensSecretTip':
      'Token 密钥仅在创建成功时展示一次，请及时复制并妥善保存；遗失后请新建 Token，并禁用或删除旧 Token。',
    'system.agentIntegration.tokenSettings': '设置',
    'system.agentIntegration.clientType': '客户端',
    'system.agentIntegration.displayPrefix': 'Token 前缀',
    'system.agentIntegration.lastUsedAt': '最后使用时间',
    'system.agentIntegration.invocationCount': '调用次数',
    'system.agentIntegration.clientOther': '其他 MCP',
    'system.agentIntegration.fullToken': '完整 Token',
    'system.agentIntegration.copyToken': '复制 Token',
    'system.agentIntegration.mcpConfig': 'MCP 配置',
    'system.agentIntegration.copyMcpConfig': '复制 MCP 配置',
    'menu.settings.organization': '组织',
    'menu.settings.organization.member': '成员',
    'menu.settings.organization.orgStructure': '组织架构',
    'menu.settings.organization.userGroup': '用户组',
    'menu.settings.organization.project': '项目',
    'menu.settings.organization.serviceIntegration': '服务集成',
    'menu.settings.organization.template': '模板',
    'menu.settings.organization.bugTemplate': '缺陷模板',
    'menu.settings.organization.templateFieldSetting': '字段设置',
    'menu.settings.organization.templateManagementList': '模板列表',
    'menu.settings.organization.templateManagementDetail': '创建模板',
    'menu.settings.organization.templateManagementCopy': '复制模板',
    'menu.settings.organization.templateManagementEdit': '更新模板',
    'menu.settings.organization.templateManagementWorkFlow': '工作流设置',
    'menu.settings.organization.log': '日志',
    'navbar.action.locale': '切换为中文',
    'system.agentIntegration.packageTitle': 'Agent 安装包管理',
    'system.agentIntegration.packageDescription': '安装包独立上传到平台文件仓库，不随 MeterSphere 后端构建发布。',
    'system.agentIntegration.packageNotice': '同一操作系统和架构只能启用一个版本；内部用户下载的是当前启用版本。',
    'system.agentIntegration.packageUpload': '上传安装包',
    'system.agentIntegration.packageEmpty': '尚未上传 Agent 安装包',
    'system.agentIntegration.packageVersion': '版本',
    'system.agentIntegration.packageVersionRequired': '请输入安装包版本',
    'system.agentIntegration.packagePlatform': '平台',
    'system.agentIntegration.packageOs': '操作系统',
    'system.agentIntegration.packageArch': '处理器架构',
    'system.agentIntegration.packageFile': '文件名',
    'system.agentIntegration.packageSize': '大小',
    'system.agentIntegration.packageDownloads': '下载次数',
    'system.agentIntegration.packageStatus': '状态',
    'system.agentIntegration.packageActivateAfterUpload': '上传后立即启用',
    'system.agentIntegration.packageDrop': '点击或拖拽 MeterSphere Agent ZIP 到此处',
    'system.agentIntegration.packageLimit': '仅支持由项目打包脚本生成的 ZIP，最大 500 MB',
    'system.agentIntegration.packageFileRequired': '请选择 Agent ZIP 安装包',
    'system.agentIntegration.packageUploadSuccess': 'Agent 安装包上传成功',
    'system.agentIntegration.packageDeleteConfirm': '确认删除 Agent 安装包 {version}？文件将同时从存储中删除。',
    ...sys,
    ...localeSettings,
    ...result,
    ...common,
  },
  dayjsLocale,
  dayjsLocaleName: 'zh-CN',
};
